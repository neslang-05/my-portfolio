      ...
      {isAdding && (
        <div className="border border-zinc-800 p-6 mb-8">
          ...
          <form onSubmit={handleAddProject} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-black border border-zinc-800 px-4 py-2 text-white focus:outline-none focus:border-zinc-600 font-sans"
                  placeholder="Project Name"
                  required
                />
              </div>
              <div>
                <label className="block text-sm font-sans text-zinc-400 mb-2">Category</label>
                <select
            ...
            <div>
              <label className="block text-sm font-sans text-zinc-400 mb-2">Description</label>
              <textarea
                value={newProject.description || ''}
                onChange={(e) => setNewProject({ ...newProject, description: e.target.value })}
                className="w-full bg-black border border-zinc-800 px-4 py-2 text-white focus:outline-none focus:border-zinc-600 h-24 font-sans"
                placeholder="Project description"
                required
              />
            </div>
            <div>
              <label className="block text-sm font-sans text-zinc-400 mb-2">GitHub URL</label>
              <input
                type="url"
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 text-sm text-zinc-400 font-sans">
                <input
                  type="checkbox"
            ...
            <div className="flex gap-2">
              <button
                type="submit"
                disabled={saving}
                className="bg-white text-black px-4 py-2 font-bold hover:bg-zinc-200 transition-colors flex items-center gap-2 disabled:opacity-60 font-sans"
              >
              ...
              <button
                ...
                className="border border-zinc-800 px-4 py-2 hover:bg-zinc-900 transition-colors font-sans"
              >
                Cancel
              </button>
            </div>
              ...
              <div key={project.id} className="p-6">
                {isEditing === project.id && editProject ? (
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-black border border-zinc-800 px-3 py-2 text-white focus:outline-none focus:border-zinc-600 font-sans"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-sans text-zinc-500 mb-2">Category</label>
                        <select
                    ...
                    <div>
                      <label className="block text-xs font-sans text-zinc-500 mb-2">Description</label>
                      <textarea
                        value={editProject.description}
                        onChange={(e) => handleEditChange('description', e.target.value)}
                        className="w-full bg-black border border-zinc-800 px-3 py-2 text-white focus:outline-none focus:border-zinc-600 h-20 font-sans"
                      />
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-sans text-zinc-500 mb-2">GitHub URL</label>
                        <input
                  ...
                  <div className="grid grid-cols-12 gap-4 items-center">
                    <div className="col-span-4">
                      <p className="font-medium font-sans">{project.title}</p>
                      <p className="text-zinc-500 text-sm truncate font-sans">{project.description}</p>
                    </div>
                    <div className="col-span-2">
                      <span className="text-xs font-sans bg-zinc-900 border border-zinc-800 px-2 py-1 text-zinc-400 uppercase">
                        {project.category}
                      </span>
                    </div>
                    <div className="col-span-3 flex flex-wrap gap-1">
                      {project.technologies.slice(0, 3).map((tech) => (
                        <span key={tech} className="text-xs text-zinc-500 font-sans">
                          {tech}
                        </span>
                      ))}
                    </div>
                    <div className="col-span-1">
                      {project.featured && <span className="text-white text-sm font-bold">★</span>}
                    </div>
